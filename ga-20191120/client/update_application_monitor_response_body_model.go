// This file is auto-generated, don't edit it. Thanks.
package client

import (
	"github.com/alibabacloud-go/tea/dara"
)

type iUpdateApplicationMonitorResponseBody interface {
	dara.Model
	String() string
	GoString() string
	SetRequestId(v string) *UpdateApplicationMonitorResponseBody
	GetRequestId() *string
}

type UpdateApplicationMonitorResponseBody struct {
	// The ID of the request.
	//
	// example:
	//
	// 04F0F334-1335-436C-A1D7-6C044FE73368
	RequestId *string `json:"RequestId,omitempty" xml:"RequestId,omitempty"`
}

func (s UpdateApplicationMonitorResponseBody) String() string {
	return dara.Prettify(s)
}

func (s UpdateApplicationMonitorResponseBody) GoString() string {
	return s.String()
}

func (s *UpdateApplicationMonitorResponseBody) GetRequestId() *string {
	return s.RequestId
}

func (s *UpdateApplicationMonitorResponseBody) SetRequestId(v string) *UpdateApplicationMonitorResponseBody {
	s.RequestId = &v
	return s
}

func (s *UpdateApplicationMonitorResponseBody) Validate() error {
	return dara.Validate(s)
}
