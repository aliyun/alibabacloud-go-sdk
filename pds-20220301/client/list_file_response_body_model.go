...
type iListFileResponseBody interface {
	...
	String() string
	GoString() string
	SetItems(v []*File) *ListFileResponseBody
	GetItems() []*File
	SetNextMarker(v string) *ListFileResponseBody
...
func (s *ListFileResponseBody) GetItems() []*File {
	return s.Items
}

func (s *ListFileResponseBody) GetNextMarker() *string {
	return s.NextMarker
}
