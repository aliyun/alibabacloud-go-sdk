// This file is auto-generated, don't edit it. Thanks.
package client

import (
	"github.com/alibabacloud-go/tea/dara"
)

type iGetSubscriptionResponse interface {
	dara.Model
	String() string
	GoString() string
	SetHeaders(v map[string]*string) *GetSubscriptionResponse
	GetHeaders() map[string]*string
	SetStatusCode(v int32) *GetSubscriptionResponse
	GetStatusCode() *int32
	SetBody(v *GetSubscriptionResponseBody) *GetSubscriptionResponse
	GetBody() *GetSubscriptionResponseBody
}

type GetSubscriptionResponse struct {
	Headers    map[string]*string           `json:"headers,omitempty" xml:"headers,omitempty"`
	StatusCode *int32                       `json:"statusCode,omitempty" xml:"statusCode,omitempty"`
	Body       *GetSubscriptionResponseBody `json:"body,omitempty" xml:"body,omitempty"`
}

func (s GetSubscriptionResponse) String() string {
	return dara.Prettify(s)
}

func (s GetSubscriptionResponse) GoString() string {
	return s.String()
}

func (s *GetSubscriptionResponse) GetHeaders() map[string]*string {
	return s.Headers
}

func (s *GetSubscriptionResponse) GetStatusCode() *int32 {
	return s.StatusCode
}

func (s *GetSubscriptionResponse) GetBody() *GetSubscriptionResponseBody {
	return s.Body
}

func (s *GetSubscriptionResponse) SetHeaders(v map[string]*string) *GetSubscriptionResponse {
	s.Headers = v
	return s
}

func (s *GetSubscriptionResponse) SetStatusCode(v int32) *GetSubscriptionResponse {
	s.StatusCode = &v
	return s
}

func (s *GetSubscriptionResponse) SetBody(v *GetSubscriptionResponseBody) *GetSubscriptionResponse {
	s.Body = v
	return s
}

func (s *GetSubscriptionResponse) Validate() error {
	if s.Body != nil {
		if err := s.Body.Validate(); err != nil {
			return err
		}
	}
	return nil
}
