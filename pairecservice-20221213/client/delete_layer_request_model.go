// This file is auto-generated, don't edit it. Thanks.
package client

import (
	"github.com/alibabacloud-go/tea/dara"
)

type iDeleteLayerRequest interface {
	dara.Model
	String() string
	GoString() string
	SetInstanceId(v string) *DeleteLayerRequest
	GetInstanceId() *string
}

type DeleteLayerRequest struct {
	// example:
	//
	// pairec-cn-abcdefg1234
	InstanceId *string `json:"InstanceId,omitempty" xml:"InstanceId,omitempty"`
}

func (s DeleteLayerRequest) String() string {
	return dara.Prettify(s)
}

func (s DeleteLayerRequest) GoString() string {
	return s.String()
}

func (s *DeleteLayerRequest) GetInstanceId() *string {
	return s.InstanceId
}

func (s *DeleteLayerRequest) SetInstanceId(v string) *DeleteLayerRequest {
	s.InstanceId = &v
	return s
}

func (s *DeleteLayerRequest) Validate() error {
	return dara.Validate(s)
}
