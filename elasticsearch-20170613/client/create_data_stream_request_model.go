// This file is auto-generated, don't edit it. Thanks.
package client

import (
	"github.com/alibabacloud-go/tea/dara"
)

type iCreateDataStreamRequest interface {
	dara.Model
	String() string
	GoString() string
	SetClientToken(v string) *CreateDataStreamRequest
	GetClientToken() *string
	SetBody(v string) *CreateDataStreamRequest
	GetBody() *string
}

type CreateDataStreamRequest struct {
	// example:
	//
	// 5A2CFF0E-5718-45B5-9D4D-70B3FF***
	ClientToken *string `json:"ClientToken,omitempty" xml:"ClientToken,omitempty"`
	Body        *string `json:"body,omitempty" xml:"body,omitempty"`
}

func (s CreateDataStreamRequest) String() string {
	return dara.Prettify(s)
}

func (s CreateDataStreamRequest) GoString() string {
	return s.String()
}

func (s *CreateDataStreamRequest) GetClientToken() *string {
	return s.ClientToken
}

func (s *CreateDataStreamRequest) GetBody() *string {
	return s.Body
}

func (s *CreateDataStreamRequest) SetClientToken(v string) *CreateDataStreamRequest {
	s.ClientToken = &v
	return s
}

func (s *CreateDataStreamRequest) SetBody(v string) *CreateDataStreamRequest {
	s.Body = &v
	return s
}

func (s *CreateDataStreamRequest) Validate() error {
	return dara.Validate(s)
}
