...
type iGrafanaWorkspaceVpcConfig interface {
	...
	SetIpSections(v string) *GrafanaWorkspaceVpcConfig
	GetIpSections() *string
...
type GrafanaWorkspaceVpcConfig struct {
	...
	//
	// 192.168.0.0/24
	IpSections *string `json:"ipSections,omitempty" xml:"ipSections,omitempty"`
	// example:
	//
	// 北京VPC-A通道
...
func (s *GrafanaWorkspaceVpcConfig) GetIpSections() *string {
	return s.IpSections
}
...
func (s *GrafanaWorkspaceVpcConfig) SetIpSections(v string) *GrafanaWorkspaceVpcConfig {
	s.IpSections = &v
	return s
}
