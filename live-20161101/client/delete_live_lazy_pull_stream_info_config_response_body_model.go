// This file is auto-generated, don't edit it. Thanks.
package client

import (
	"github.com/alibabacloud-go/tea/dara"
)

type iDeleteLiveLazyPullStreamInfoConfigResponseBody interface {
	dara.Model
	String() string
	GoString() string
	SetRequestId(v string) *DeleteLiveLazyPullStreamInfoConfigResponseBody
	GetRequestId() *string
}

type DeleteLiveLazyPullStreamInfoConfigResponseBody struct {
	// The request ID.
	//
	// example:
	//
	// 16A96B9A-F203-4EC5-8E43-CB92E68F4CD8
	RequestId *string `json:"RequestId,omitempty" xml:"RequestId,omitempty"`
}

func (s DeleteLiveLazyPullStreamInfoConfigResponseBody) String() string {
	return dara.Prettify(s)
}

func (s DeleteLiveLazyPullStreamInfoConfigResponseBody) GoString() string {
	return s.String()
}

func (s *DeleteLiveLazyPullStreamInfoConfigResponseBody) GetRequestId() *string {
	return s.RequestId
}

func (s *DeleteLiveLazyPullStreamInfoConfigResponseBody) SetRequestId(v string) *DeleteLiveLazyPullStreamInfoConfigResponseBody {
	s.RequestId = &v
	return s
}

func (s *DeleteLiveLazyPullStreamInfoConfigResponseBody) Validate() error {
	return dara.Validate(s)
}
