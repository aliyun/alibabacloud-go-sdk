// This file is auto-generated, don't edit it. Thanks.
package client

import (
	"github.com/alibabacloud-go/tea/dara"
)

type iStopCasterResponseBody interface {
	dara.Model
	String() string
	GoString() string
	SetRequestId(v string) *StopCasterResponseBody
	GetRequestId() *string
}

type StopCasterResponseBody struct {
	// The request ID.
	//
	// example:
	//
	// 16A96B9A-F203-4EC5-8E43-CB92E68F4CD8
	RequestId *string `json:"RequestId,omitempty" xml:"RequestId,omitempty"`
}

func (s StopCasterResponseBody) String() string {
	return dara.Prettify(s)
}

func (s StopCasterResponseBody) GoString() string {
	return s.String()
}

func (s *StopCasterResponseBody) GetRequestId() *string {
	return s.RequestId
}

func (s *StopCasterResponseBody) SetRequestId(v string) *StopCasterResponseBody {
	s.RequestId = &v
	return s
}

func (s *StopCasterResponseBody) Validate() error {
	return dara.Validate(s)
}
