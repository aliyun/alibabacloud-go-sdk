// This file is auto-generated, don't edit it. Thanks.
package client

import (
	"github.com/alibabacloud-go/tea/dara"
)

type iUpdateNatGatewayNatTypeResponseBody interface {
	dara.Model
	String() string
	GoString() string
	SetRequestId(v string) *UpdateNatGatewayNatTypeResponseBody
	GetRequestId() *string
}

type UpdateNatGatewayNatTypeResponseBody struct {
	// The request ID.
	//
	// example:
	//
	// 0ED8D006-F706-4D23-88ED-E11ED28DCAC0
	RequestId *string `json:"RequestId,omitempty" xml:"RequestId,omitempty"`
}

func (s UpdateNatGatewayNatTypeResponseBody) String() string {
	return dara.Prettify(s)
}

func (s UpdateNatGatewayNatTypeResponseBody) GoString() string {
	return s.String()
}

func (s *UpdateNatGatewayNatTypeResponseBody) GetRequestId() *string {
	return s.RequestId
}

func (s *UpdateNatGatewayNatTypeResponseBody) SetRequestId(v string) *UpdateNatGatewayNatTypeResponseBody {
	s.RequestId = &v
	return s
}

func (s *UpdateNatGatewayNatTypeResponseBody) Validate() error {
	return dara.Validate(s)
}
