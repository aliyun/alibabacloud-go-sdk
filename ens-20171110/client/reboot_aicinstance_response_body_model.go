// This file is auto-generated, don't edit it. Thanks.
package client

import (
	"github.com/alibabacloud-go/tea/dara"
)

type iRebootAICInstanceResponseBody interface {
	dara.Model
	String() string
	GoString() string
	SetRequestId(v string) *RebootAICInstanceResponseBody
	GetRequestId() *string
}

type RebootAICInstanceResponseBody struct {
	// The ID of the request.
	//
	// example:
	//
	// 473469C7-AA6F-4DC5-B3DB-A3DC0DE3C83E
	RequestId *string `json:"RequestId,omitempty" xml:"RequestId,omitempty"`
}

func (s RebootAICInstanceResponseBody) String() string {
	return dara.Prettify(s)
}

func (s RebootAICInstanceResponseBody) GoString() string {
	return s.String()
}

func (s *RebootAICInstanceResponseBody) GetRequestId() *string {
	return s.RequestId
}

func (s *RebootAICInstanceResponseBody) SetRequestId(v string) *RebootAICInstanceResponseBody {
	s.RequestId = &v
	return s
}

func (s *RebootAICInstanceResponseBody) Validate() error {
	return dara.Validate(s)
}
