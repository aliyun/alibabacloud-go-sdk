// This file is auto-generated, don't edit it. Thanks.
package client

import (
	"github.com/alibabacloud-go/tea/dara"
)

type iGetModelInputContentDetectResultResponse interface {
	dara.Model
	String() string
	GoString() string
	SetHeaders(v map[string]*string) *GetModelInputContentDetectResultResponse
	GetHeaders() map[string]*string
	SetStatusCode(v int32) *GetModelInputContentDetectResultResponse
	GetStatusCode() *int32
	SetBody(v *GetModelInputContentDetectResultResponseBody) *GetModelInputContentDetectResultResponse
	GetBody() *GetModelInputContentDetectResultResponseBody
}

type GetModelInputContentDetectResultResponse struct {
	Headers    map[string]*string                            `json:"headers,omitempty" xml:"headers,omitempty"`
	StatusCode *int32                                        `json:"statusCode,omitempty" xml:"statusCode,omitempty"`
	Body       *GetModelInputContentDetectResultResponseBody `json:"body,omitempty" xml:"body,omitempty"`
}

func (s GetModelInputContentDetectResultResponse) String() string {
	return dara.Prettify(s)
}

func (s GetModelInputContentDetectResultResponse) GoString() string {
	return s.String()
}

func (s *GetModelInputContentDetectResultResponse) GetHeaders() map[string]*string {
	return s.Headers
}

func (s *GetModelInputContentDetectResultResponse) GetStatusCode() *int32 {
	return s.StatusCode
}

func (s *GetModelInputContentDetectResultResponse) GetBody() *GetModelInputContentDetectResultResponseBody {
	return s.Body
}

func (s *GetModelInputContentDetectResultResponse) SetHeaders(v map[string]*string) *GetModelInputContentDetectResultResponse {
	s.Headers = v
	return s
}

func (s *GetModelInputContentDetectResultResponse) SetStatusCode(v int32) *GetModelInputContentDetectResultResponse {
	s.StatusCode = &v
	return s
}

func (s *GetModelInputContentDetectResultResponse) SetBody(v *GetModelInputContentDetectResultResponseBody) *GetModelInputContentDetectResultResponse {
	s.Body = v
	return s
}

func (s *GetModelInputContentDetectResultResponse) Validate() error {
	if s.Body != nil {
		if err := s.Body.Validate(); err != nil {
			return err
		}
	}
	return nil
}
