// This file is auto-generated, don't edit it. Thanks.
package client

import (
	"github.com/alibabacloud-go/tea/dara"
)

type iChatDocumentPageNum interface {
	dara.Model
	String() string
	GoString() string
	SetNum(v int32) *ChatDocumentPageNum
	GetNum() *int32
	SetPos(v [][]*ChatRefDocPostion) *ChatDocumentPageNum
	GetPos() [][]*ChatRefDocPostion
}

type ChatDocumentPageNum struct {
	Num *int32                 `json:"num,omitempty" xml:"num,omitempty"`
	Pos [][]*ChatRefDocPostion `json:"pos,omitempty" xml:"pos,omitempty" type:"Repeated"`
}

func (s ChatDocumentPageNum) String() string {
	return dara.Prettify(s)
}

func (s ChatDocumentPageNum) GoString() string {
	return s.String()
}

func (s *ChatDocumentPageNum) GetNum() *int32 {
	return s.Num
}

func (s *ChatDocumentPageNum) GetPos() [][]*ChatRefDocPostion {
	return s.Pos
}

func (s *ChatDocumentPageNum) SetNum(v int32) *ChatDocumentPageNum {
	s.Num = &v
	return s
}

func (s *ChatDocumentPageNum) SetPos(v [][]*ChatRefDocPostion) *ChatDocumentPageNum {
	s.Pos = v
	return s
}

func (s *ChatDocumentPageNum) Validate() error {
	return dara.Validate(s)
}
