// This file is auto-generated, don't edit it. Thanks.
package client

import (
	"github.com/alibabacloud-go/tea/dara"
)

type iCreateExperimentMigrateValidationResponseBody interface {
	dara.Model
	String() string
	GoString() string
	SetRequestId(v string) *CreateExperimentMigrateValidationResponseBody
	GetRequestId() *string
}

type CreateExperimentMigrateValidationResponseBody struct {
	// example:
	//
	// 473469C7-AA6F-4DC5-B3DB-A3DC0DE3C83E
	RequestId *string `json:"RequestId,omitempty" xml:"RequestId,omitempty"`
}

func (s CreateExperimentMigrateValidationResponseBody) String() string {
	return dara.Prettify(s)
}

func (s CreateExperimentMigrateValidationResponseBody) GoString() string {
	return s.String()
}

func (s *CreateExperimentMigrateValidationResponseBody) GetRequestId() *string {
	return s.RequestId
}

func (s *CreateExperimentMigrateValidationResponseBody) SetRequestId(v string) *CreateExperimentMigrateValidationResponseBody {
	s.RequestId = &v
	return s
}

func (s *CreateExperimentMigrateValidationResponseBody) Validate() error {
	return dara.Validate(s)
}
